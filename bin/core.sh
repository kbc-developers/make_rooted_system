...
# -------------------------------------------------------
export BASE_DIR=$1
export apktool=$BASE_DIR/bin/apktool
export _MODEL_SEL=$2
export _BUILD_SEL=$3
...
func_extract_factoryfs_files()
{
	...
	if [ -d $_FACTORYFS_DIR ]; then
		rm -rf $_FACTORYFS_DIR
	fi
	mkdir -p $_FACTORYFS_DIR

	./bin/ext4_utils/simg2img $_FACTORYFS_IMG $_TMP_DIR/output.img
	mount -o loop,ro,noexec,noload $_TMP_DIR/output.img $_TMP_DIR/mnt
	rsync -av $_TMP_DIR/mnt/ $_FACTORYFS_DIR
	umount $_TMP_DIR/mnt
}
# -------------------------------------------------------
...
func_repack_factoryfs_files()
{
	...
	cd ./bin/ext4_utils
	./make_ext4fs -s -l $_FACTORYFS_IMG_SIZE -a system $_FACTORYFS_IMG $_FACTORYFS_DIR
	cd $BASE_DIR
}
# -------------------------------------------------------
